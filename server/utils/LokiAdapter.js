...
const LokiAdapter = function LokiAdapter() {

    /**
     * A modifier
     */
    const maCollection = 'maCollection';

    this.init = function () {
        return new Promise((success) => {
            db.loadDatabase({}, () => {
                createIfNotExist(maCollection);
                success(db);
            });
        });
    };

    this.getMaCollection = function () {
        return db.getCollection(maCollection);
    };
    ...
    let dbPath = path.join(homedir, 'my-app');
    ...
    if (LokiAdapter.caller !== LokiAdapter.getInstance) {
 ...
 ************************************************************************ */
LokiAdapter.instance = null;

/**
 * Singleton getInstance definition
 * @return LokiAdapter class
 */
LokiAdapter.getInstance = function () {
    if (this.instance === null) {
        this.instance = new LokiAdapter();
    }
    return this.instance;
};

module.exports = LokiAdapter.getInstance();